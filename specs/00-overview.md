...
| D19 | Trivial-tier verification | Self-confidence gate replaces the judge at trivial tier only: the model appends a verbalized `"CONFIDENCE: X"` line to its own answer, parsed by `quality_gate.parse_verbalized_confidence` — no second model call, no provider API feature required. (Originally speced around provider `logprobs`; abandoned after confirming Groq rejects that parameter on every model, with no timeline — a hardware-level fact, not a rare gap, so verbalized confidence replaced it outright rather than as a per-model fallback.) Rationale: judge model's own footprint (`wh_per_1k_out: 0.3`) exceeds the trivial model's (`wh_per_1k_out: 0.12`) — judging every trivial answer was taxing the cheapest tier more than the tier's own answer cost. Moderate tier keeps the independent judge (D8/D9) since an unjudged wrong answer there costs real quality or an unnecessary Opus hop. Fallback to independent judge if the answer has no parseable confidence line. |
